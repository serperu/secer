...
get_type_inside_erl(ArgList)->
  {Args, Analysis_} = analyze_args(ArgList, #args{}, #analysis{}),
  %% if the mode has not been set, set it to the default mode (show)
  Analysis=
     case Analysis_#analysis.mode of
     undefined -> Analysis_#analysis{mode = ?SHOW};
     Mode when is_atom(Mode) -> Analysis_
   end,
  % io:format("Args: ~p\n", [Args]),
...
no_show(Analysis,[{File, Module}|Files])->
   ...
   [no_show_type_info(Info)|no_show(Analysis,Files)].

no_show_type_info(Info) ->
  no_show_type_info(Info#info.functions,Info).

no_show_type_info([],_)->[];
no_show_type_info([{_LineNo, F, A}|Funs],Info)->
  %[get_type_string(F, A, Info, show)|no_show_type_info(Funs,Info)].
  [{F,A,get_type_info({F,A}, Info#info.types),get_type_string(F, A, Info, show)}|no_show_type_info(Funs,Info)].
