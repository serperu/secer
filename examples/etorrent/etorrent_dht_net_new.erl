...
-import(etorrent_bcoding_new, [get_value/2, get_value/3]).
...
get_string(What, PL) ->
    etorrent_bcoding_new:get_string_value(What, PL).
...
-spec decode_msg(string()) -> any().
decode_msg(InMsg) ->
    io:format("1: ~p\n", [InMsg]),
    Msg = etorrent_bcoding_new:decode(InMsg),
    io:format("1: ~p\n", [Msg]),
...
encode_query(Method, MsgID, Params) ->
    Msg = [
       ...
       {<<"t">>, MsgID},
       {<<"a">>, Params}],
    etorrent_bcoding_new:encode(Msg).
...
prova(X) ->
    % Enc = iolist_to_binary(etorrent_bcoding:encode(X)), 
