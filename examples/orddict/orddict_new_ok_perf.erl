...
-export([from_list_replicate/1, discard_small/1]).
...
from_list([{_,_}]=Pair) -> 
    Start = os:timestamp(),
    Res = Pair,
    {Pair, timer:now_diff(os:timestamp(), Start)},
    Res;
from_list(Pairs) ->
    Start = os:timestamp(),
    Res = lists:ukeysort(1, reverse_pairs(Pairs, [])),
    {Pairs, timer:now_diff(os:timestamp(), Start)},
    Res.
...
-spec from_list_replicate(List) -> Orddict when
      List :: [{Key :: term(), Value :: term()}],
      Orddict :: orddict().

from_list_replicate(Pairs) ->
  from_list(replicate(100, Pairs, [])).

replicate(1, List, Acc) ->
  Acc ++ List;
replicate(N, List, Acc) ->
  NList = lists:map(fun inc/1, List),
  replicate(N - 1, NList, Acc ++ List).

inc({X, V}) ->
  {{1, X}, V}.

discard_small(L) when length(L) > 5 ->  
    io:format("CONSIDERED\n"),
    from_list(L);
discard_small(L) ->
    [].
