...
main(T1, T2) ->
    ...
    Tape1 = turing(fun incrementer_config/0, fun incrementer/2, T1),
    % io:format("~w~n", [Tape1]),
 
    Tape2 = turing(fun busy_beaver_config/0, fun busy_beaver/2, T2),
    %io:format("~w~n", [Tape2]).
    {Tape1, Tape2}.
...
perform(Config, Rules, State, Input = {LeftInput, RightInput}) ->
    ...
    case lists:member(State, Halts) of
        true  -> Input; %RIGHT
        % true  -> LeftInput; %WRONG -> NOT CONSIDERED
        false ->
